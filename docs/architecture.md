...
Shared alarms and timers carry `label_color` as an opaque signed ARGB integer, or `0` for the theme-aware Default choice. Default is used for new items and resolves to Charcoal in light mode and Snow in dark mode on each device. Stored explicit colors, including Snow (`-1`), remain unchanged. Updates that omit the field preserve the stored color. Saved timer templates retain the same value, and timer adjustments forward it with the rest of the shared configuration. Deploy the server migration and API update before distributing clients that send the Default value.
